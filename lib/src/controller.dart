...
class VideoEditorController extends ChangeNotifier {
  ...
  final ValueNotifier<CoverData?> _selectedCover = ValueNotifier<CoverData?>(
    null,
  );
  ...
  void cropAspectRatio(double? value) {
    ...
    if (value != null) {
      ...
      _minCrop = Offset(
        centerCrop.left / videoWidth,
        centerCrop.top / videoHeight,
      );
      _maxCrop = Offset(
  ...
  void updateTrim(double min, double max) {
    ...
    final Duration newDurationFloor = Duration(
      microseconds: newDuration.floor(),
    );
    assert(
